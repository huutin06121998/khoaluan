...
$(document).on("click",".canam",function(){
  var mahs = $(this).attr("mahs");
  var tenhs = $(this).attr("tenhs");
  Swal.fire({
      title: 'Thực hiện tổng kết năm học của học sinh '+tenhs,
      type: 'success',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.value) {
          $.ajax({
             async: false,
             type: "delete",
             url: "./tongkethocky/result",
             data: {mahs:mahs,tenhs:tenhs},
             success: function(response){
                 if(response == "1")
                  {
                      Swal.fire(
                          'Bạn thực hiện thành công',
                          'Bạn đã tổng kết năm học',
                          'success'
                      ).then(function(){
                          //reload
                          location.reload();
...
$(document).on('click','#button_excel',function () {
  var key_sr = $("#Search").val();
    $.ajax({
      async: false,
      type: "post",
      url: "./tongkethocky/data_showall",
      data:{
            key_sr:key_sr
        },
      success: function (response) {
...
$(document).on('click','#exp',function () {
  $("#tb_exp").table2excel({
    exclude : ".noExl",
    name    : "Danh sách điểm tổng kết học kỳ",
    filename: "Danhsachdiemhocky"
  }); 
});